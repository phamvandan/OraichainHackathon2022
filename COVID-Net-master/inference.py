...
parser.add_argument('--weightspath', default='models/COVIDNet-CXR-S', type=str, 
...
parser.add_argument('--imagepath', default='../demo_cxr', type=str, help='Full path to image to be inferenced')
...
filenames = os.listdir(args.imagepath)
f = open('../demo_cxr.txt', 'a+')
for filename in filenames:
    path = os.path.join(args.imagepath, filename)
    if args.is_medusa_backbone:
        x = process_image_file(path, args.input_size, top_percent=0, crop=False)
        x = x.astype('float32') / 255.0
        medusa_image_tensor = graph.get_tensor_by_name(args.in_tensorname_medusa)
        medusa_x = process_image_file_medusa(path, args.input_size_medusa)
        feed_dict = {
                    medusa_image_tensor: np.expand_dims(medusa_x, axis=0),
                    image_tensor: np.expand_dims(x, axis=0),
                }
    else:
        x = process_image_file(path, args.input_size, top_percent=args.top_percent)
        x = x.astype('float32') / 255.0
        feed_dict = {image_tensor: np.expand_dims(x, axis=0)}

    pred = sess.run(pred_tensor, feed_dict=feed_dict)
    result = filename + '\t' +inv_mapping[pred.argmax(axis=1)[0]] + '\t' + str(pred[0][pred.argmax(axis=1)[0]])
    print(result)
